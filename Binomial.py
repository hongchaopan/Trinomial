# This file is writtedn for calculating the value of American options via trinomial tree method based on binomial tree method
# This file is written for calculating the value of American option via binomial tree method,
...
def Binomial_American(S,K,T,sigma,q,r,N,option_type=None):
    '''
    Calculate the value of American option
    :param S:       Spot price
    ...
    for j in range(N-1,-1,-1): # N-1:0
        for k in range(2*j+1):
            V_put[k]=max(math.exp(-r*delta_t)*(V_put[k]*pRN+V_put[k+1]*pm+V_put[k+2]*qRN),(K-S*math.pow(u,(j-k))))
            V_call[k] = max(math.exp(-r * delta_t) * (V_call[k] * pRN + V_call[k + 1] * pm+V_call[k+2]*qRN),
                            -(K - S * math.pow(u, (j - k))))
...
def Average_binomial_American(S,K,T,sigma,q,r,N,option_type=None):
    '''
...
def BBS_American(t,S,K,T,sigma,q,r,N,option_type=None):
    '''
    Calculate the value of options via TBS method
    :param t:       start time
    :param S:       Spot price
    :param K:       Strike price
    :param T:       Maturity
    ...
    for i in range(2*N-1):  # 2*(N-1)+1
        ...
        V_put[i] = max(BS.black_scholes(t,S_t,K,delta_t,sigma,r,q,"PUT"), (K - math.pow(u, (N-1 - i))* S))
        V_call[i] = max(BS.black_scholes(t,S_t,K,delta_t,sigma,r,q,"CALL"), -(K - math.pow(u, (N-1 - i)) * S))

    for j in range(N - 2, -1, -1):  # N-2:0
        for k in range(2*j + 1):
            V_put[k] = max(math.exp(-r * delta_t) * (V_put[k] * pRN + V_put[k + 1] * pm+V_put[k+2]*qRN),
                           (K - S * math.pow(u, (j - k))))
            V_call[k] = max(math.exp(-r * delta_t) * (V_call[k] * pRN + V_call[k + 1] * pm+V_call[k+2]*qRN),
                            -(K - S * math.pow(u, (j - k))))
        # Get the values for greeks
...
def BBSR_American(t,S,K,T,sigma,q,r,N,option_type=None):
    '''
    ...
    '''
    V_BBS1,delta1,gamma1,theta1=BBS_American(t,S,K,T,sigma,q,r,N,option_type)
    V_BBS2, delta2, gamma2, theta2 = BBS_American(t, S, K, T, sigma, q, r, int(N/2), option_type)
    V_BBSR=(2*V_BBS1-V_BBS2)
    delta_BBSR=(2*delta1-delta2)
    gamma_BBSR=(2*gamma1-gamma2)
    theta_BBSR=(2*theta1-theta2)

    #V_BBSR=2*BBS_American(t,S,K,T,sigma,q,r,N,option_type)-BBS_American(t,S,K,T,sigma,q,r,N/2,option_type)
...
if __name__ == "__main__":
    ...
    # Get the exact value
    V_exact,delta_exact,gamma_exact,theta_exact=Average_binomial_American(S,K,T,sigma,q,r,N,"PUT")
    print("The exact value is: ", V_exact,delta_exact,gamma_exact,theta_exact)

    v,delta,gamma,theta=BBS_American(t,S,K,T,sigma,q,r,20,"PUT")
